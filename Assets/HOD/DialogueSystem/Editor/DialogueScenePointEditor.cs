...
public class DialogueScenePointEditor : Editor
{
    ...
    private void DrawMainMenu()
    {
        ...
        GUILayout.BeginHorizontal();
        GUILayout.Label("проигрывание со старта:");
        EditorGUI.BeginChangeCheck();
        point.playOnStart = EditorGUILayout.Toggle(point.playOnStart);
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(point, "Изменена настройка проигрывания на старте" + name);
            EditorUtility.SetDirty(point);
        }
        GUILayout.EndHorizontal();
