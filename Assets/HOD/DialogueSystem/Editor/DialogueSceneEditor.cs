...
public class DialogueSceneEditor : EditorWindow
{
    ...
    private void DrawEvent(EventNode eventNode, Rect windowRect)
    {
        ...
        {
            bufer = new Rect(5, 43, windowRect.width - 5, 40);

            string characterName = eventNode.character.name;
            string characterStatName = eventNode.character.characterStats[eventNode.changeCharacterStatIndex].statName;
            string statValue = eventNode.changeCharacterStatValue.ToString();

            if(eventNode.changeCharacterStatValue > 0)
            {
                statValue = "+" + statValue;
            }

            string s = characterName + "\n" + characterStatName + " " + statValue;

            EditorGUI.LabelField(bufer, s);
        }
    }
    private void DrawLink(LinkNode linkNode, Rect nodeTransform)
    {
        ...
        {
            //bufer = new Rect(nodeTransform.width - 42, 1, 24, 20);
            //if (GUI.Button(bufer, "tp"))
            //{
            //    scrollViewRect = new Rect(linkNode.transformRect.position, scrollViewRect.size);
            //}
